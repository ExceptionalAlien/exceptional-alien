...
export async function GET(request: Request, { params }: { params: { destination: string } }) {
  ...
  const destination = await client.getByUID("destination", params.destination, {
    ...
    fetchLinks:
      "playbook.sub_title,playbook.app_title,playbook.image,playbook.locked,playbook.creator,creator.first_name,creator.last_name,creator.title,playbook.destination,destination.title",
  });
