...
export async function GET(request: Request, { params }: { params: { uid: string } }) {
  ...
  const playbook = await client.getByUID("playbook", params.uid, {
    ...
    fetchLinks:
      "creator.first_name,creator.last_name,creator.title,creator.profile_image,destination.title,gem.title,gem.category,gem.description,gem.address,gem.location,gem.image,gem.destination,gem.playbooks",
  });
