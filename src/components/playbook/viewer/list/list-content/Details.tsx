...
export default function ListContent({ description, audio }: { description: RichTextField; audio: string }) {
  return (
    <section className="grid grid-cols-1 gap-y-5 md:gap-y-6 [&>p]:text-base [&>p]:text-ex-grey">
      ...
      <Link
        href="/"
        className="text-ex-blue leading-none hover:bg-ex-blue hover:text-white duration-300 ease-in-out transition-[background-color, color] rounded-full p-2 pl-4 pr-4 border-[1.5px] border-ex-blue flex items-center justify-center md:w-max"
      >
