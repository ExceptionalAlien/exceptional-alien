...
export default function GemIcon(props: GemIconProps) {
  return (
    <div
      className={`gem-icon absolute flex h-9 w-9 items-center justify-center md:h-11 md:w-11 [&>svg]:absolute [&>svg]:text-ex-blue [&>svg]:transition-[color] [&>svg]:duration-300 [&>svg]:ease-in-out ${
        props.hideBg ? "[&>svg]:h-full" : "[&>svg]:h-2/5"
      } ${props.classes}`}
    >
      <BG className={`gem-icon-bg !h-full !text-white ${props.hideBg && "hidden"}`} />
      <Border className={`!h-full ${props.hideBg && "hidden"}`} />
      ...
      {props.category === "Food & Drink" && (
        <svg
          ...
          fill="currentColor"
          data-name="Layer 1"
          viewBox="0 0 17 18.09"
          className="gem-icon-symbol"
        >
          <path d="M4 18.09H3V8.56c-.74-.09-1.42-.4-1.95-.88C.39 7.09 0 6.26 0 5.41V.09h1v5.32c0 .57.25 1.11.71 1.52.35.31.8.53 1.29.61V.09h1v7.46c.49-.08.95-.3 1.3-.62.45-.41.7-.95.7-1.53V.09h1v5.32c0 .85-.38 1.67-1.03 2.27-.54.49-1.23.8-1.97.89v9.53Zm-.5-10h.02-.02Zm13 9.5h-6v-1H13V9.57c-1.56-.15-3-1.03-3-2.57 0-1.38 1-6.76 1.04-6.99l.98.18C11.67 2.1 11 5.97 11 7c0 1.17 1.5 1.6 2.5 1.6S16 8.17 16 7C16 5.71 14.99.24 14.98.19l.98-.19C16 .23 17 5.62 17 6.99c0 1.54-1.44 2.42-3 2.57v7.02h2.5v1Z" />
        </svg>
      )}
      ...
      {props.category === "Culture" && (
        <svg
          ...
          className="gem-icon-symbol mb-[2px] md:mb-[3px]"
        >
