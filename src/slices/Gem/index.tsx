...
interface Creator {
  uid: string;
  data: Content.CreatorDocumentData;
}
 ...
 * Props for `Gem`.
 */
export type GemProps = SliceComponentProps<Content.GemSlice, Context>;
...
const Gem = ({ slice, context }: GemProps): JSX.Element => {
  return (
    ...
    >
      <GemIcon category={(slice.primary.gem as Gem).data.category} classes="right-0 !w-9 md:!w-11" />
      ...
      <div className="mt-2 md:mt-3">
        <Image
          ...
          width={(slice.primary.gem as Gem).data.image.dimensions!.width}
          height={(slice.primary.gem as Gem).data.image.dimensions!.height}
          placeholder={`data:image/svg+xml;base64,${toBase64(
            shimmer(
              (slice.primary.gem as Gem).data.image.dimensions!.width,
              (slice.primary.gem as Gem).data.image.dimensions!.height
            )
          )}`}
        ...
        <div className="float-left pb-6 mb-6 md:mb-8 pl-3 md:pl-4 w-3/5 [&>*]:text-ex-blue [&>*]:text-sm [&>*]:md:text-base [&>*]:font-bold [&>*]:leading-snug [&>*]:md:!leading-normal">
          ...
          {!(slice.primary.creator as Creator).data ? (
            ...
            <Link
              href={`/creators/${
                (slice.primary.creator as Creator).data ? (slice.primary.creator as Creator).uid : context.creator.uid
              }`}
              className="block max-w-full w-max mt-3 md:mt-4 hover:opacity-50 duration-300 ease-in-out transition-opacity"
            >
              {(slice.primary.creator as Creator).data.first_name}{" "}
              {(slice.primary.creator as Creator).data.last_name?.toUpperCase()}
              <Image
                src={
                  (slice.primary.creator as Creator).data
                    ? ((slice.primary.creator as Creator).data.profile_image.url as string)
                    : (context.creator.data.profile_image.url as string)
                }
                alt={
                  (slice.primary.creator as Creator).data
                    ? ((slice.primary.creator as Creator).data.profile_image.alt as string)
                ...
                className={`inline-block rounded-full ml-2 w-8 md:w-10 align-[-11px] md:align-[-14px] ${
                  !(slice.primary.creator as Creator).data.last_name && "hidden"
                }`}
              />
            </Link>
          )}
